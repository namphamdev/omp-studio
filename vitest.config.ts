...
//   - Vitest `include` matches ONLY `src/renderer/**/*.test.ts(x)`.
//   - bun's `bunfig.toml` pins `[test] root = "test"`, so bun never walks
//     `src/renderer` and never sees these renderer suites.
...
export default defineConfig({
  ...
  test: {
    ...
    // ONLY renderer tests. Keeps Vitest off the bun `test/` node suites and
    // the Playwright `e2e/` specs (*.spec.ts).
